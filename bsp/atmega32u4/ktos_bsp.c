...
/**
 * @file ktos_bsp.c
 * @brief KTOS Board Support Package — ATmega32U4 (AVR 8-bit)
 *
 * Supported boards: Arduino Leonardo, Arduino Micro, Arduino Pro Micro.
 *
 * | Property  | Value                              |
 * |-----------|------------------------------------|
 * | Core      | AVR 8-bit                          |
 ...
 * | Toolchain | avr-gcc                            |
 *
 * The ATmega32U4 has a 2-byte program counter (22 KB flash range uses
 * only 14 address bits).  The stack frame is identical to the ATmega328P.
 * Timer1 registers (TCCR1A/B, OCR1A, TIMSK1) are the same as on the 328P.
 *
 * ### Build
 * @code
 ...
 * @endcode
 *
 * @defgroup ktos_bsp_avr32u4 KTOS BSP — AVR (ATmega32U4)
 * @ingroup  ktos_hal
 */
 ...
 * Timer1 on the ATmega32U4 uses the same register layout as the ATmega328P.
 ...
 *
 * The ATmega32U4 has a 2-byte program counter, identical to the ATmega328P.
 * Stack layout (top = lowest address):
 * ```
 * [ R31..R1, R0 ]  32 GPRs
 * [ SREG ]         status register (I-bit set)
 * [ exit PC high ] 2-byte return address
 * [ exit PC low  ]
 * [ task PC high ] 2-byte task entry point
 * [ task PC low  ]
 * ```
 */
...
/* =========================================================================
 * Context switch — AVR assembly (identical to ATmega328P)
 * ========================================================================= */
