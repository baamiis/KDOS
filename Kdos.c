...
static int32_t *OS_SP; // Corrected line
...
// queue size (max number of messages). These parameters are implicitly checkedby
...
// The second parameter is the message type. This can be any value from 0-65535and the
// message types are enumerated in KmCDOS.h. Note that 0 is pre-defined by CDOSas
...
// structure and passed to the receiving task. The meanings of these parametersare
...
// a successful operation would call WakeUp and a timeout would occur at the endof the
...
// Recursive. Entered in system mode, partly executed in user mode. Called onlyby
...
static void SwitchTask()
{
  ...
  while (TRUE)
  {
    ...
    { // Not sleeping, no msgs, check timer
      TaskCurrent->Timer = 0;
      TaskCurrent->TimerFlag = FALSE;
      OS_SP = SP;                 // Save system stack
      SP = TaskCurrent->StackPtr; // Load user stack
...
// always will be enabled upon exit). This is to allow TaskCurrent->Sleeping tobe set
// after an I/O function has been initiated but before an interrupt can call WakeUp() to
// check the status of this flag.
// The first parameter specifies the number of milliseconds to sleep, from 0 to65534.
...
// There are 2 special cases. A sleep time of 0 will cause no specific delay butgives
// the opportunity to all other tasks to execute if they are ready. This shouldbe used
