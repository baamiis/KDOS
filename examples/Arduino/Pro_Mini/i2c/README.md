# KTOS — Arduino Pro Mini I²C Scanner Example (Bare-Metal)
...
| Device pin | Arduino label | AVR pin |
|------------|------------|---------|
| `SDA`      | `A4`       | `PC4`   |
| `SCL`      | `A5`       | `PC5`   |
| `VCC`      | `5V`       | —       |
...
```
   Pro Mini A4 (PC4) ── SDA ── device
   Pro Mini A5 (PC5) ── SCL ── device
   Pro Mini VCC       ── VCC ── device
   Pro Mini GND      ── GND ── device
```

> **Pull-ups:** TWI lines are open-drain.  If your module doesn't have
> built-in pull-ups, add 4.7 kΩ resistors from each line to `5V`.

> **3.3 V devices:** the Pro Mini drives the bus at 5 V.  Use a level
...
cd KTOS/examples/Arduino/Pro_Mini/i2c
```
