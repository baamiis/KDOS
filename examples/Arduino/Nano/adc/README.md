...
| Potentiometer pin | Nano label | AVR pin |
|-------------------|------------|---------|
| End 1             | `5V`       | —       |
...
floating-point `printf`, which is too large for the Nano's flash.
...
cd KTOS/examples/Arduino/Nano/adc
```
...
| Voltage shows ≈ `0.49` × actual     | AVcc reference is not 5 V               | Power the Nano from USB or a regulated 5 V source         |
